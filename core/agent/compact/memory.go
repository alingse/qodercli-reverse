...
func LogCompactResult(result *CompactResult) {
	log.Info("Compact completed:",
		" original_messages=%d", result.OriginalMessageCount,
		" compressed_messages=%d", result.CompressedMessageCount,
		" saved_tokens=%d", result.SavedTokens,
		" compression_ratio=%.2f%%", result.CompressionRatio,
		" strategy=%v", result.Strategy)
}
